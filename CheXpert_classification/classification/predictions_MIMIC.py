from classification.dataset_MIMIC  import MIMICCXRDataset
import pandas as pd
import torch
...
def make_pred_multilabel(s, model, test_df, val_df, PATH_TO_IMAGES, device):
    ...
    dataset_test = MIMICCXRDataset(test_df, PATH_TO_IMAGES, transform=transforms.Compose([
    ...
    dataset_val = MIMICCXRDataset(val_df, PATH_TO_IMAGES, transform=transforms.Compose([
        transforms.Resize(256),
        transforms.CenterCrop(256),
        transforms.ToTensor(),
        normalize]))
    ...
    for mode in ["Threshold", "test"]:
        ...
        if mode == "test":
            ...
            Eval = pd.read_csv("./results" + s + "/Threshold.csv")
...
#                 bi_pred = bi_pred_df["bi_" + column]
        ...
        pred_df.to_csv("results" + s + "/preds.csv", index=False)
        true_df.to_csv("results" + s + "/True.csv", index=False)

        if mode == "Threshold":
            Eval_df.to_csv("results" + s + "/Threshold.csv", index=False)

        if mode == "test":
            TestEval_df.to_csv("results" + s + "/TestEval.csv", index=False)
            bi_pred_df.to_csv("results" + s + "/bipred.csv", index=False)
